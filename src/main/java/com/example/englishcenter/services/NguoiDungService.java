...
import com.example.englishcenter.services.iservices.INguoiDungService;
...
public class NguoiDungService implements INguoiDungService {
    ...
    public void updateDiemHocVien(Long mahocvien, DiemHocVienResponse diemHocVienResponse) {
        ...
        Float diemKiemTra = diemHocVienResponse.getDiemkiemtra();
        Float diemCuoiKi = diemHocVienResponse.getDiemdiemcuoiki();

        exitsNlh.setDiemkiemtra(diemKiemTra);
        exitsNlh.setDiemcuoiki(diemCuoiKi);

        if (diemKiemTra != null && diemCuoiKi != null) {
            float diemTrungBinh = (diemKiemTra + diemCuoiKi) / 2;

            if (diemTrungBinh >= 4.0f) {
                exitsNlh.setTrangThai("Đạt");
            } else {
                exitsNlh.setTrangThai("Không Đạt");
            }
        } else {
            exitsNlh.setTrangThai("Đang Học");
        }
    ...
    public void deleteNguoiDung(long MaNguoiDung) {
        nguoiLopHocRepository.deleteByMaNguoiDung(MaNguoiDung);
    ...
    public List<NguoiDung> getALLGiaoVien() {
        return nguoiDungRepository.findAllGiaoVien("Giáo Viên");
    }
