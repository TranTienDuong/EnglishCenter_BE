...
public interface ThoiKhoaBieuRepository extends JpaRepository<ThoiKhoaBieu, Long> {
    ...
    @Query("SELECT CASE WHEN COUNT(tkb) > 0 THEN true ELSE false END FROM ThoiKhoaBieu tkb " +
            "WHERE tkb.lophoc IN (SELECT nl.lopHoc FROM NguoiLopHoc nl WHERE nl.nguoiDung.manguoidung = :maGV) " +
            "AND tkb.thuhoc IN :thuHoc " +
            "AND tkb.tgbatdau = :tgbatdau AND tkb.tgketthuc = :tgketthuc")
    boolean coLichTrung(@Param("maGV") Long maGV,
                        @Param("thuHoc") List<Integer> thuHoc,
                        @Param("tgbatdau") LocalTime tgbatdau,
                        @Param("tgketthuc") LocalTime tgketthuc);
    ...
    @Query("SELECT tkb FROM ThoiKhoaBieu tkb " +
            "JOIN tkb.lophoc lh " +
            "JOIN lh.nguoiLopHocs nlh " +
            "WHERE nlh.nguoiDung.manguoidung = :maNguoiDung " +
            "AND nlh.trangThai <> 'Chuyển Lớp'")
